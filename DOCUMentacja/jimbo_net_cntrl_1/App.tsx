...
import { generateAgentReport, analyzeConnectionSecurity } from './services/geminiService';
...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'arch' | 'metrics'>('dashboard');
  ...
  const addAgentLog = useCallback((agentId: string, message: string) => {
    setAgents(prev => prev.map(a => 
      a.id === agentId ? { ...a, log: [message, ...a.log].slice(0, 10), status: AgentStatus.WORKING, lastActivity: 'Teraz' } : a
    ));
    setTimeout(() => {
  ...
  const handleTogglePersistence = (id: string) => {
    ...
    setTunnels(prev => prev.map(t => t.id === id ? { ...t, isPersistent: !t.isPersistent } : t));
  ...
  const handleCreateTunnel = (port: number, serviceName: string, obfuscate: boolean) => {
    const randomSuffix = Math.random().toString(36).substring(2, 7);
    const obfuscatedId = obfuscate ? `cf-${randomSuffix}-${Math.floor(Math.random() * 99)}` : `srv-${serviceName.toLowerCase()}`;
  ...
  const handleAdvancedCreateTunnel = (config: { label: string; port: number; provider: 'Cloudflare' | 'ngrok' | 'Local'; obfuscate: boolean }) => {
    const randomSuffix = Math.random().toString(36).substring(2, 7);
    ...
    <div className="flex h-screen overflow-hidden bg-black text-slate-300">
      ...
      <aside className="w-64 bg-zinc-950 border-r border-slate-900 flex flex-col shrink-0">
        <div className="p-8">
          ...
          <p className="text-[10px] text-blue-500 font-bold uppercase tracking-[0.2em] mt-2 glow-text">Control Center v1.2</p>
        </div>

        <nav className="flex-1 px-4 space-y-1">
          <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded text-sm transition-all ${activeTab === 'dashboard' ? 'bg-blue-600/10 text-blue-400 border-l-2 border-blue-600' : 'text-slate-500 hover:text-white'}`}>Dashboard</button>
          <button onClick={() => setActiveTab('arch')} className={`w-full flex items-center gap-3 px-4 py-3 rounded text-sm transition-all ${activeTab === 'arch' ? 'bg-blue-600/10 text-blue-400 border-l-2 border-blue-600' : 'text-slate-500 hover:text-white'}`}>Infrastruktura</button>
          <button onClick={() => setActiveTab('metrics')} className={`w-full flex items-center gap-3 px-4 py-3 rounded text-sm transition-all ${activeTab === 'metrics' ? 'bg-blue-600/10 text-blue-400 border-l-2 border-blue-600' : 'text-slate-500 hover:text-white'}`}>Metryki</button>
          <button onClick={() => setActiveTab('tools')} className={`w-full flex items-center gap-3 px-4 py-3 rounded text-sm transition-all ${activeTab === 'tools' ? 'bg-blue-600/10 text-blue-400 border-l-2 border-blue-600' : 'text-slate-500 hover:text-white'}`}>Narzędzia</button>
        </nav>
      ...
      <main className="flex-1 overflow-y-auto bg-[#020202]">
        <header className="sticky top-0 z-20 bg-black/80 backdrop-blur-xl border-b border-slate-900 p-6 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-white uppercase tracking-wider">{activeTab === 'dashboard' ? 'Command Hub' : activeTab === 'arch' ? 'Architektura' : activeTab === 'metrics' ? 'Metryki Użytkowania' : 'Narzędzia Systemowe'}</h2>
            <p className="text-[9px] text-slate-600 uppercase font-black tracking-widest mt-1">Interfejs bezpośredniej orkiestracji</p>
          </div>
          <div className="flex gap-4">
             <button onClick={() => setIsModalOpen(true)} className="px-5 py-2.5 bg-blue-600 text-white text-[10px] font-black uppercase rounded hover:bg-blue-500 transition-all shadow-lg flex items-center gap-2">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 4v16m8-8H4" /></svg>
                Dodaj Tunel
             </button>
             <button onClick={handleOpenTask} className="px-5 py-2.5 bg-white text-black text-[10px] font-black uppercase rounded hover:bg-slate-200 transition-all shadow-lg flex items-center gap-2">
              ...
              <div className="col-span-12 lg:col-span-8 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <TunnelStatus 
                    ...
                    onToggle={(id) => {
                      const tunnel = tunnels.find(t => t.id === id);
                      setTunnels(prev => prev.map(t => t.id === id ? {...t, isActive: !t.isActive} : t));
                       ...
                       {agents.map(agent => (
                         <div key={agent.id} className="space-y-2">
                            <div className="flex justify-between items-center text-[10px]">
                               ...
                               <span className={`px-1.5 py-0.5 rounded font-black ${agent.status === AgentStatus.WORKING ? 'bg-green-600/20 text-green-500' : 'text-slate-700'}`}>{agent.status}</span>
                            </div>
                            <div className="bg-black/40 border border-slate-900 p-3 rounded text-[11px] leading-relaxed max-h-[120px] overflow-y-auto custom-scrollbar">
                               {agent.log.map((line, idx) => (
                                 <div key={idx} className={`${idx === 0 ? 'text-slate-300 font-bold' : 'text-slate-600'} italic`}>
                                   {idx === 0 ? '> ' : '  '} {line}
                                 </div>
                               ))}
                            </div>
                      ...
                      {services.map(s => (
                        <div key={s.pid} className="group flex justify-between items-center p-2 rounded hover:bg-white/5 transition-all">
                           ...
                           <div className="flex gap-2">
                             <div className={`w-1.5 h-1.5 rounded-full self-center ${s.isExposed ? 'bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'bg-blue-500'}`}></div>
                             <button onClick={() => handleKillTask(s.pid)} className="opacity-0 group-hover:opacity-100 p-1 text-slate-600 hover:text-red-500 transition-all">
