...
const App: React.FC = () => {
  ...
  // Fallback to mock if agent not available
  const [mockTunnels, setMockTunnels] = useState<TunnelConfig[]>(MOCK_TUNNELS);
  const tunnels = agentConnection.isConnected
    ? agentTunnels.map((t) => ({
        id: t.id,
        label: t.name,
        obfuscatedId: t.id,
