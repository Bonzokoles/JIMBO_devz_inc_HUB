...
async function handleOrchestrate(request: Request, env: Env): Promise<Response> {
    ...
    const { query, context = {}, model = 'deepseek/deepseek-r1' } = await request.json();
    ...
    // Store task status
    await env.AGENT_STATE.put(`task:${taskId}`, JSON.stringify({
        taskId,
        query,
        plan,
        status: 'executing',
...
async function executePlan(
    ...
    const tasksByPriority = plan.tasks.reduce((acc, task) => {
    ...
    for (const priority of priorities) {
        ...
        console.log(`[${taskId}] Executing ${tasks.length} tasks at priority ${priority}`);
...
async function callAI(
    ...
    // Use OpenRouter (supports DeepSeek and many other models)
