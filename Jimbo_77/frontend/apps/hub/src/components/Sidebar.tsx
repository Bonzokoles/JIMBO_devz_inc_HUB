...
import { motion, AnimatePresence } from 'framer-motion'; // Assuming framer-motion might be added, or fallback to CSS classes
...
const menuItems = [
  ...
  { id: 'marketplace', label: 'MARKET', icon: '🛒', category: 'SYSTEM', external: true, url: 'https://ai-marketplace.stolarnia-ams.workers.dev' },
];
    ...
    <div className="w-80 h-full flex flex-col border-r border-glass-border bg-glass-bg backdrop-blur-xl shadow-2xl z-20 transition-all duration-300">
      ...
      <div className="p-8 border-b border-glass-border relative overflow-hidden group">
        ...
        <div className="flex items-center space-x-2 mt-2">
           ...
           <span className="text-xs font-mono tracking-[0.2em] text-jimbo-gold/80 group-hover:text-jimbo-gold transition-colors">DEVZ_INC HUB</span>
        </div>
      </div>
          ...
          <div key={category} className="mb-6">
            ...
            {menuItems.filter(item => item.category === category).map((item) => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => item.external ? window.open(item.url, '_blank') : setActiveTab(item.id)}
                  className={`
                ...
                >
                  ...
                  <span className={`text-xl transition-transform duration-300 ${isActive ? 'scale-110 drop-shadow-md' : 'group-hover:scale-110 grayscale group-hover:grayscale-0'}`}>
                    {item.icon}
                  </span>
                  ...
                  {/* Hover Glow Effect */}
                  <div className={`absolute inset-0 rounded-lg bg-gradient-to-r from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none ${isActive ? 'hidden' : ''}`} />
                </button>
              );
            })}
          </div>
      ...
      <div className="p-4 border-t border-glass-border bg-black/20">
        <div className="glass-panel rounded-xl p-3 flex items-center space-x-3 hover:bg-white/5 transition-colors cursor-pointer group">
          ...
          <div className="flex-1">
            <div className="text-sm font-bold text-white group-hover:text-jimbo-gold transition-colors">Admin User</div>
