...
export function MultiDomainAnalyticsView() {
  ...
  const exportToCSV = () => {
    const csv = [
      ["Domain", "Monthly Clicks", "Current Traffic", "Conversion Rate", "AI Crawlers", "Revenue"],
      ...domains.map(d => [
        d.domain,
        d.monthlyClicks,
        d.currentTraffic,
    ...
    a.href = url;
    a.download = `domain-analytics-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  };

  const totalTraffic = domains.reduce((sum, d) => sum + d.currentTraffic, 0);
  const totalRevenue = domains.reduce((sum, d) => sum + d.revenue, 0);
  const avgConversion = domains.reduce((sum, d) => sum + d.conversionRate, 0) / domains.length;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-glass-border">
        <div>
          ...
          <p className="font-mono text-sm text-gray-400 tracking-wide mt-2">
            ...
            <span className="text-jimbo-gold ml-2">{totalTraffic.toLocaleString()}</span> TOTAL
            VISITS •
            <span className="text-green-400 ml-2 shadow-green-900/50 drop-shadow-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {domains.map(domain => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm text-gray-300 font-mono">
                      <span
                        className={`${domain.conversionRate > 3 ? "text-green-400" : "text-gray-400"}`}
                      >
                    ...
                    <td className="p-4 pr-6 text-right font-mono text-sm">
                      <span
                        className={
                          domain.revenue > 0
                            ? "text-green-400 drop-shadow-sm font-bold"
                            : "text-gray-600"
                        }
                      >
                        ${domain.revenue}
